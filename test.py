...
def current_reference(Y1, cout, pixel, prob_residual, codebook_residual=None, codebook_motion=None):
    ...
    for i, Ry in enumerate(Rys):
        ...
        if codebook_residual:
            compressed += "".join(entropyCoding(codebook_residual,
                                                reordered_Ry))
...
def writefile(Y, U, V, outstream):
    def inverse_repeat(a, repeats, axis):
        if isinstance(repeats, int):
            indices = np.arange(
                a.shape[axis] / repeats, dtype=np.int) * repeats
...
def readfile():
    ...
    for done_prob in [False, True]:
        ...
        while True:
            ...
            U2 = U.copy()
            V2 = V.copy()
            Y1 = current_reference(
                Y2, cout, pix, prob_residual, codebook_residual, codebook_motion)
...
def decodeFrame(ref, residual):
    ...
    for resi in residual:
        inverse_reordered_Ry = inverseReorder([resi])

        Dequantised_Ry = rescale(inverse_reordered_Ry)
...
def decode(height, width, pix):
    ...
    while idx < len(compressed):
        idx, residual = huffmanDecode(
            height, width, pix, codebook_residual, compressed, idx)
